    ...
    <div class="solve" id="solve2-1">
      ...
      <div class="set-routeName">
        ...
        <div class="set-routeName-default">
          <input class="set-routeName-default-input" type="text" placeholder="请填写公交的部分名称" v-model="stationName">//v-model?
        </div>
        <button type = "button"  class = "search-button" v-on:click="getStationByRoutePartialName()"> 查询 </button>
      </div>
      <div style="height:300px; width:80%; background-color: white">
        <el-scrollbar style="height:100%; width: 100%">

          <ul class="result-ul">
            <li class="result-li" v-for="(item,index) in oneRouteAllStationInfo" :key="index">
              <StationItem1 class="route-item" :stationInfo="item" :index="index"></StationItem1>
            </li>
          </ul>
        </el-scrollbar>
      </div>
...
<!--    </div>-->
    ...
    <div class="solve" id="solve2-3">
      ...
      <div class="set-routeName">
        ...
        <div class="set-routeName-default">
          <input class="set-routeName-default-input" type="text" placeholder="请填写公交名称" v-model="stationName">//v-model改成？
        </div>
        <button type = "button"  class = "search-button" v-on:click="getOneStationAllRoute()"> 查询 </button>
      </div>
      <div style="height:300px; width:80%; background-color: white">
        <el-scrollbar style="height:100%; width: 100%">

          <ul class="result-ul">
            <li class="result-li" v-for="(item,index) in oneStationAllRoutes" :key="index">
              <StationItem3 class="route-item" :routeInfo="item" :index="index"></StationItem3>
            </li>
          </ul>
        </el-scrollbar>
      </div>
    ...
    //
    // 2.4
    <div class="solve" id="solve2-4">
      <div class="solve-title">
        <p class ="solve-title-p" >根据两站＆线路部分名称获取线路的运行方向、沿途站点、运行时长</p></div>
      <div class="set-routeName">
        <button type = "button"  class = "search-button" @click="multipleQueryBetweenTwoStations()"> 查询 </button>
      </div>
      <div style="height:300px; width:80% ; background-color: white">
        <el-scrollbar style="height:100%; width: 100%">
    ...
    <div class="solve" id="solve2-6">
      ...
      <div class="set-routeName"> // 改为stationName
        ...
        <div class="set-routeName-default">
          <input class="set-routeName-default-input" type="text" placeholder="请填写站点1的名称" v-model="stationName261">//改v-model
        </div>
        <div class="set-routeName-default">
          <input class="set-routeName-default-input" type="text" placeholder="请填写站点2的名称" v-model="stationName262">//改v-model
        </div>
        <button type = "button"  class = "search-button" v-on:click="judgeIfDirect()"> 查询 </button>
      </div>
      <div style="height:300px; width:80%; background-color: white">
        <el-scrollbar style="height:100%; width: 100%">
          <StationItem6 class="route-item" :directInfo="directInfo" ></StationItem6> // class改？
        </el-scrollbar>
      </div>
    </div>
    //
